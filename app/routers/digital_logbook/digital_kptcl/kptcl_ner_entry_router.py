...
router = APIRouter(
    prefix="/kptcl-ner-entry",
    tags=["KPTCL NER Entry"],dependencies=[Depends(validate_token)]
)
...
class KPTCLNEREntryCreate(BaseModel):
...
class KPTCLNEREntryUpdate(KPTCLNEREntryCreate):
    pass
...
@router.post("")
def create_kptcl_ner_entry(
    payload: KPTCLNEREntryCreate,
    db: Session = Depends(get_db)
):
    query = text("""
        INSERT INTO kptcl_ner_entry (
            master_id,
            reading_date,
            reading_time,
    ...
    return {
        "message": "KPTCL NER Entry created successfully",
...
def get_kptcl_ner(
    ...
    master_sql = text("""
        SELECT *
        FROM kptcl_ner_master knm
        WHERE knm.ms_logbook_id IN (
    ...
    if not masters:
        return {
            "date": str(parsed_date),
            "module": "kptcl_ner",
            "message": "No KPTCL NER found for this date",
            "kptcl_ner": None
        }

    # 3️⃣ Collect all kptcl_ner_ids and fetch entries
    kptcl_ner_ids = [m["kptcl_ner_id"] for m in masters]

    entry_sql = text("""
        SELECT *
        FROM kptcl_ner_entry
        WHERE master_id = ANY(:kptcl_ner_ids)
        ORDER BY master_id, reading_date, reading_time
    """)

    entries = db.execute(
        entry_sql,
        {"kptcl_ner_ids": kptcl_ner_ids}
    ).mappings().all()
...
# GET by entry_id
# ─────────────────────────────────────────────
@router.get("/kptcl-ner/entry/{kptcl_ner_entry_id}")
def get_kptcl_ner_entry(
    kptcl_ner_entry_id: int,
    db: Session = Depends(get_db)
):
    ...
    entry_sql = text("""
        SELECT *
        FROM kptcl_ner_entry
        WHERE kptcl_ner_id = :kptcl_ner_entry_id
    """)

    entry = db.execute(
        entry_sql,
        {"kptcl_ner_entry_id": kptcl_ner_entry_id}
    ).mappings().first()

    if not entry:
        raise HTTPException(
            status_code=404,
            detail=f"KPTCL NER entry with id {kptcl_ner_entry_id} not found"
        )
...
@router.put("/{entry_id}")
def update_kptcl_ner_entry(
    entry_id: int,
    payload: KPTCLNEREntryUpdate,
    db: Session = Depends(get_db)
):
    query = text("""
        UPDATE kptcl_ner_entry
        SET
            master_id = :master_id,
    ...
    if result.rowcount == 0:
        raise HTTPException(status_code=404, detail="KPTCL NER entry not found")

    return {"message": "KPTCL NER Entry updated successfully"}
...
def delete_kptcl_ner_entry(
    entry_id: int,
    db: Session = Depends(get_db)
):
    query = text("""
        DELETE FROM kptcl_ner_entry
        WHERE kptcl_ner_id = :entry_id
    """)
    ...
    return {"message": "KPTCL NER Entry deleted successfully"}
