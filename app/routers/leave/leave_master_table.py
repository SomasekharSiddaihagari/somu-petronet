...
def get_leave_summary(user_id: int, db: Session = Depends(get_db)):
    ...
    # ------------------------------------------------------------
    # 2. Fetch applied leaves → Taken + Provisioned (WITH 7-DAY WINDOW)
    # ------------------------------------------------------------
        ...
        SELECT 
            ...
            SUM(CASE 
                    WHEN LOWER(status) IN ('approved', 'reversal approved') 
                         AND to_date <= (CURRENT_DATE - INTERVAL '7 days')  -- ✅ 7-day window
                    THEN number_of_days 
                    ELSE 0 
                END) AS taken_days,
            SUM(CASE 
    ...
    for row in balances:
        ...
        # Track encashable/non_encashable per row
    ...
    # ------------------------------------------------------------
    # 5. OVERRIDE Earned Leave (ALSO WITH 7-DAY WINDOW)
    # ------------------------------------------------------------
    if "Earned Leave" in merged:
        ...
        # ✅ Apply 7-day window for taken leaves
        ...
        # Encashments still in-flight
        pending_encashed = float(db.execute(text("""
            SELECT COALESCE(SUM(le.encash_el), 0)
            FROM leave_encashment le
            LEFT JOIN encashment_main em ON em.encashment_main_id = le.encashment_main_id
            WHERE LOWER(le.status) NOT IN (
                'encashment approved', 'payment processed', 'paid',
                'supervisor rejected', 'rejected', 'cancelled','auto encashed'
            )
            AND (le.created_by = :uid OR em.created_by = :uid)
        """), {"uid": user_id}).scalar() or 0)
        ...
        # Actual balance blocks double-applying by deducting pending encashments
        actual_encashable_balance = max(encashable - pending_encashed, 0.0)
        ...
        el["pending_encashment"]      = pending_encashed           
        el["actual_balance"]          = actual_encashable_balance  

    # ------------------------------------------------------------
    # 6. Summary totals (ALSO WITH 7-DAY WINDOW)
    # ------------------------------------------------------------
    ...
    return {
        ...
        "summary": {
            ...
            "total_pending": float(total_provisioned)
        }
    }
