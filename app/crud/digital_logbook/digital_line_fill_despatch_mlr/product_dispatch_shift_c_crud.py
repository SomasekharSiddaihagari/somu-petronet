# app/crud/erv_c_shift_log_crud.py
...
def create_erv_c_shift_log(db: Session, payload: ERVCShiftLogCreate):
    query = text("""
        INSERT INTO erv_c_shift_log (
            category_master_id,
...
def update_erv_c_shift_log(
    ...
    data["erv_c_shift_log_id"] = erv_c_shift_log_id

    query = text(f"""
        UPDATE erv_c_shift_log
        SET {set_clause}
        WHERE erv_c_shift_log_id = :erv_c_shift_log_id
    """)
...
def delete_erv_c_shift_log(db: Session, erv_c_shift_log_id: int):
    query = text("""
        DELETE FROM erv_c_shift_log
        WHERE erv_c_shift_log_id = :erv_c_shift_log_id
    """)
    result = db.execute(query, {"erv_c_shift_log_id": erv_c_shift_log_id})
    db.commit()
