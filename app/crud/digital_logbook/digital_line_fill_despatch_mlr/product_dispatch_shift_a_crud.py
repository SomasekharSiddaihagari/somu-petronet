# app/crud/erv_a_shift_log_crud.py
...
def update_erv_a_shift_log(
    ...
    data["a_shift_log_id"] = a_shift_log_id

    query = text(f"""
        UPDATE a_shift_log
        SET {set_clause}
        WHERE a_shift_log_id = :a_shift_log_id
    """)
...
def delete_erv_a_shift_log(db: Session, a_shift_log_id: int):
    query = text("""
        DELETE FROM a_shift_log
        WHERE a_shift_log_id = :a_shift_log_id
    """)
    result = db.execute(query, {"a_shift_log_id": a_shift_log_id})
    db.commit()
