# app/crud/digital_logbook/digital_cp_reading/cp_reading_ner_entry_crud.py
from sqlalchemy.orm import Session
from sqlalchemy import text
from app.schemas.digital_logbook.digital_cp_reading.cp_reading_ner_entry_schema import (
    CPReadingNEREntryCreate,
    CPReadingNEREntryUpdate,
)

from datetime import datetime


def get_ner_entries_by_master_id(db: Session, mid: int):
    """Fetch NER entries for a master ID with dynamic name resolution."""
    query = text(
        """
        SELECT 
            e.*,
...
def get_ner_entry_by_id(db: Session, eid: int):
    """Retrieve a single NER entry with dynamic name resolution."""
    query = text(
        """
        SELECT 
            e.*,
            ...
            (SELECT TRIM(CONCAT(first_name, ' ', last_name)) FROM users WHERE user_id = e.updated_by) as updated_by_name
        FROM cp_reading_ner_entry e 
        WHERE e.cp_ner_entry_id = :id
    """
    )
    return db.execute(query, {"id": eid}).mappings().first()


def create_ner_entry(db: Session, payload: CPReadingNEREntryCreate, uid: int):
    data = payload.model_dump(exclude_unset=True)

    # Exclude primary key
    data.pop("cp_ner_entry_id", None)
    ...
    res = db.execute(
        text(
            f"INSERT INTO cp_reading_ner_entry ({cols}) VALUES ({vals}) RETURNING cp_ner_entry_id"
        ),
        data,
    )
    db.commit()
...
def update_ner_entry(db: Session, eid: int, payload: CPReadingNEREntryUpdate, uid: int):
    old = get_ner_entry_by_id(db, eid)
    if not old:
        return False
    ...
    db.execute(
        text(f"INSERT INTO cp_reading_ner_entry_history ({cols}) VALUES ({vals})"),
        history_data,
    )
    ...
    query = text(
        f"UPDATE cp_reading_ner_entry SET {set_clause} WHERE cp_ner_entry_id = :eid"
    )
...
def delete_ner_entry(db: Session, eid: int, uid: int):
    old = get_ner_entry_by_id(db, eid)
    if not old:
        return False
    ...
    db.execute(
        text("DELETE FROM cp_reading_ner_entry WHERE cp_ner_entry_id = :eid"),
        {"eid": eid},
    )
    db.commit()
    return True
