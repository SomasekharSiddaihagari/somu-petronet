# app/crud/digital_logbook/digital_cp_reading/cp_reading_mlr_entry_crud.py
...
def get_mlr_entries_by_master_id(db: Session, mid: int):
    """Fetch MLR entries for a master ID with dynamic name resolution."""
    query = text(
        """
        SELECT 
            e.*,
...
def get_mlr_entry_by_id(db: Session, eid: int):
    """Retrieve a single MLR entry with dynamic name resolution."""
    query = text(
        """
        SELECT 
            e.*,
            ...
            (SELECT TRIM(CONCAT(first_name, ' ', last_name)) FROM users WHERE user_id = e.updated_by) as updated_by_name
        FROM cp_reading_mlr_entry e 
        WHERE e.cp_mlr_entry_id = :id
    """
    )
    return db.execute(query, {"id": eid}).mappings().first()


def create_mlr_entry(db: Session, payload: CPReadingMLREntryCreate, uid: int):
    data = payload.model_dump(exclude_unset=True)

    # Exclude primary key
    data.pop("cp_mlr_entry_id", None)
    ...
    res = db.execute(
        text(
            f"INSERT INTO cp_reading_mlr_entry ({cols}) VALUES ({vals}) RETURNING cp_mlr_entry_id"
        ),
        data,
    )
    db.commit()
...
def update_mlr_entry(db: Session, eid: int, payload: CPReadingMLREntryUpdate, uid: int):
    old = get_mlr_entry_by_id(db, eid)
    if not old:
        return False
    ...
    db.execute(
        text(f"INSERT INTO cp_reading_mlr_entry_history ({cols}) VALUES ({vals})"),
        history_data,
    )
    ...
    query = text(
        f"UPDATE cp_reading_mlr_entry SET {set_clause} WHERE cp_mlr_entry_id = :eid"
    )
...
def delete_mlr_entry(db: Session, eid: int, uid: int):
    old = get_mlr_entry_by_id(db, eid)
    if not old:
        return False
    ...
    db.execute(
        text("DELETE FROM cp_reading_mlr_entry WHERE cp_mlr_entry_id = :eid"),
        {"eid": eid},
    )
    db.commit()
    return True
