...
def get_dkn_entries_by_master_id(db: Session, mid: int):
    """Fetch DKN entries for a master ID with dynamic name resolution."""
    query = text(
        """
        SELECT 
            e.*,
...
def get_dkn_entry_by_id(db: Session, eid: int):
    """Retrieve a single DKN entry with dynamic name resolution."""
    query = text(
        """
        SELECT 
            e.*,
            ...
            (SELECT TRIM(CONCAT(first_name, ' ', last_name)) FROM users WHERE user_id = e.updated_by) as updated_by_name
        FROM cp_reading_dkn_entry e 
        WHERE e.cp_dkn_entry_id = :id
    """
    )
    return db.execute(query, {"id": eid}).mappings().first()


def create_dkn_entry(db: Session, payload: CPReadingDKNEntryCreate, uid: int):
    data = payload.model_dump(exclude_unset=True)

    # Exclude primary key
    data.pop("cp_dkn_entry_id", None)
    ...
    res = db.execute(
        text(
            f"INSERT INTO cp_reading_dkn_entry ({cols}) VALUES ({vals}) RETURNING cp_dkn_entry_id"
        ),
        data,
    )
    db.commit()
...
def update_dkn_entry(db: Session, eid: int, payload: CPReadingDKNEntryUpdate, uid: int):
    old = get_dkn_entry_by_id(db, eid)
    if not old:
        return False
    ...
    db.execute(
        text(f"INSERT INTO cp_reading_dkn_entry_history ({cols}) VALUES ({vals})"),
        history_data,
    )
    ...
    db.execute(
        text(
            f"UPDATE cp_reading_dkn_entry SET {set_clause} WHERE cp_dkn_entry_id = :eid"
        ),
        params,
    )
    db.commit()
...
def delete_dkn_entry(db: Session, eid: int, uid: int):
    old = get_dkn_entry_by_id(db, eid)
    if not old:
        return False
    ...
    db.execute(
        text("DELETE FROM cp_reading_dkn_entry WHERE cp_dkn_entry_id = :eid"),
        {"eid": eid},
    )
    db.commit()
    return True
