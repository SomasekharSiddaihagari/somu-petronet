# app/crud/digital_logbook/digital_cp_reading/cp_reading_mlr_master_crud.py
...
def _log_history(db: Session, record: Dict[str, Any], action: str, user_id: int = None):
    """Archive a snapshot of the MLR master record."""
    ...
    query = text(f"INSERT INTO cp_reading_mlr_master_history ({cols}) VALUES ({vals})")
    db.execute(query, history_data)


def _process_mlr_result(db: Session, result):
    if not result:
        return None
    d = dict(result)
    ...
    d["entries"] = (
        get_mlr_entries_by_master_id(db, d["cp_mlr_id"]) if d.get("cp_mlr_id") else []
    )
    return d


def get_mlr_master_by_id(db: Session, mlr_id: int):
    """Retrieve a MLR master record with dynamic name resolution."""
    query = text(
        """
        SELECT 
            m.*,
            ...
            COALESCE(m.station, 'Mangalore') as station
        FROM cp_reading_mlr_master m
        WHERE m.cp_mlr_id = :id
    """
    )
    result = db.execute(query, {"id": mlr_id}).mappings().first()
    return _process_mlr_result(db, result)


def create_mlr_master(db: Session, payload: CPReadingMLRMasterCreate, user_id: int):
    data = payload.model_dump(exclude_unset=True)
    for field in ["cp_mlr_id", "entries"]:
        data.pop(field, None)
    ...
    query = text(
        f"INSERT INTO cp_reading_mlr_master ({cols}) VALUES ({vals}) RETURNING cp_mlr_id"
    )
...
def update_mlr_master(
    db: Session, mlr_id: int, payload: CPReadingMLRMasterUpdate, user_id: int
):
    """Update a MLR master record and log history snapshot."""
    old_record = get_mlr_master_by_id(db, mlr_id)
    if not old_record:
        return False
    ...
    query = text(f"UPDATE cp_reading_mlr_master SET {set_clause} WHERE cp_mlr_id = :id")
    db.execute(query, {**data, "id": mlr_id})
    db.commit()
    return True


def delete_mlr_master(db: Session, mlr_id: int, user_id: int):
    """Perform archival deletion of a MLR master record."""
    old_record = get_mlr_master_by_id(db, mlr_id)
    if not old_record:
        return False
    ...
    db.execute(text("DELETE FROM cp_reading_mlr_master WHERE cp_mlr_id = :id"), {"id": mlr_id})
    db.commit()
    return True


def get_mlr_masters_by_date_range(db: Session, from_date: date, to_date: date):
    """Retrieve MLR master records within a date range with dynamic name resolution."""
    query = text(
        """
        SELECT m.*,
               (SELECT TRIM(CONCAT(first_name, ' ', last_name)) FROM users WHERE user_id = m.created_by) as created_by_name,
        ...
        WHERE m.log_date >= :from_date
          ...
          AND EXISTS (SELECT 1 FROM cp_reading_mlr_entry e WHERE e.master_id = m.cp_mlr_id)
        ORDER BY m.log_date DESC, m.cp_mlr_id DESC
    """
    )
    results = (
        db.execute(query, {"from_date": from_date, "to_date": to_date}).mappings().all()
    )
    return [_process_mlr_result(db, row) for row in results]
