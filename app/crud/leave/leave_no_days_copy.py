...
def validate_hpl(data):
    if data["days"] > 2 and not data["has_med_cert"]:
        return False, "HPL > 2 days requires medical certificate."
    return True, None
...
def get_allocated_leave(db, user_id, leave_code):
    ...
    allocated = (
        db.query(LeaveBalance.allocated)
...
# def get_used_leave_days(db, user_id, leave_code):

#     used = (
#         db.query(func.count(HRLeaveApplicationDay.leave_day_id))
#         .join(
#             HRLeaveApplication,
#             HRLeaveApplication.leave_id ==
#             HRLeaveApplicationDay.leave_application_id
#         )
#         .filter(
#             HRLeaveApplication.user_id == user_id,
#             func.lower(HRLeaveApplication.leave_type) == leave_code.lower(),
#             HRLeaveApplication.status.ilike("%approved%")
#         )
#         .scalar()
#     )
...
#     reversal_to
# ):
#     # Step 1: Delete the reversed day rows
#     db.execute(text("""
...
#         # ✅ Release comp-off days linked to this application
#         db.execute(text("""
...
#         # ✅ Release ONLY the reversed comp-off days (partial reversal)
#         db.execute(text("""
#             UPDATE hr_leave_compof_day_new
#             SET is_used = FALSE,
#                 leave_application_id = NULL
...
def approve_reversal(
    ...
    if not remaining_rows:
        # Full reversal — all leave days removed
        db.execute(text("""
            UPDATE hr_leave_application
            SET number_of_days = 0,
                from_date      = :rf,
        ...
        # ✅ Full reversal → release ALL comp-off days linked to this application
        # Don't filter by date — comp-off dates are earned dates, not leave dates
        db.execute(text("""
            UPDATE hr_leave_compof_day_new
            SET is_used = FALSE,
                leave_application_id = NULL
        ...
        # ✅ Partial reversal — figure out how many days were reversed
        # and release that many comp-off days (oldest first)
        reversed_count = db.execute(text("""
            SELECT COUNT(*) FROM hr_leave_application_day
            WHERE leave_application_id = :leave_id
              AND leave_date BETWEEN :rf AND :rt
        """), {
            "leave_id": leave_id,
        ...
        # NOTE: This count was done BEFORE delete in Step 1,
        # so fetch from comp-off table directly
        reversed_day_count = int(
            (reversal_to - reversal_from).days + 1
        )

        # Release comp-off days equal to reversed count (oldest first)
        db.execute(text("""
            UPDATE hr_leave_compof_day_new
            SET is_used = FALSE,
                leave_application_id = NULL
...
def get_used_leave_days(db: Session, user_id: int, leave_type: str) -> float:
    ...
    sql = """
        SELECT COUNT(*)::float
        FROM hr_leave_application_day d
        JOIN hr_leave_application h
          ON h.leave_id = d.leave_application_id
        WHERE h.user_id = :uid
