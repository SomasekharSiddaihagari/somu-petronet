...
def is_rejected(column):
    """
    Returns SQLAlchemy condition:
    TRUE only if the CLAIM itself was rejected (not buyback rejection)
    """
    return func.lower(column).like("%claim rejected%") | func.lower(column).like("%auto lapsed%")
# -------------------------------------------------
...
,
    ...
    # =================================================
        ...
        total_utilized = (
            ...
            .filter(                                            # ✅ FILTER AFTER
                AssetClaim.created_by == user_id,              # ✅ AssetClaim.created_by (not Submission)
        ...
        bought_back_amount = (
            ...
            .filter(
                AssetClaim.created_by == user_id,              # ✅ AssetClaim.created_by
                AssetClaim.category == "Furniture",
                AssetClaim.bought_back.is_(True),
        ...
        # --------------------------------------------------
        # 4. LAST CLAIM DATE (non-rejected only)
        # --------------------------------------------------
        last_claim = (
            db.query(AssetClaim)
            .join(                                              # ✅ JOIN added
            ...
            .filter(
                AssetClaim.created_by == user_id,
                AssetClaim.category == category,
                ~is_rejected(AssetClaimSubmission.status),     # ✅ exclude rejected
            )
            .order_by(AssetClaim.claim_date.desc())
            .first()
        )
