...
# ✅ Helper to get CURRENT FY (still needed for reference if required elsewhere)
...
def validate_furniture_rm(payload, db: Session):
    ...
    # Get FY range based on the claim_month_year user sent
    fy_start, fy_end = get_financial_year_range(payload.claim_month_year)

    # -----------------------------------------------------
    # ONLY VALIDATION KEPT: Duplicate Claim in same FY
    # -----------------------------------------------------
    existing_claim = (
        ...
        .filter(
            ...
            ~func.lower(FurnitureRMReimbursement.status).like('%rejected%') | ~func.lower(FurnitureRMReimbursement.status).like('%auto lapsed%')
        )
        .first()
    )
...
def validate_vehicle_cm(payload, db: Session):
    ...
    if not user:
        return ["Invalid user"], None, flags

    if payload.employee_grade not in ENTITLEMENTS:
        return ["Invalid employee grade"], None, {}
    ...
    active_vehicle = (
        ...
        .filter(
            UserVehicle.user_id == payload.user_id,
            UserVehicle.active == True
        )
        .first()
    )

    if not active_vehicle:
        return ["No active vehicle found"], None, {}

    vehicle_fuel_type = active_vehicle.fuel_type
    flags = {
    "active_vehicle_found": True,
    "active_vehicle_fuel_type": active_vehicle.fuel_type,
    "vehicle_type": active_vehicle.vehicle_type,
    "fixed_conveyance": payload.fixed_conveyance_claim,
}
    ...
    # RC + INSURANCE
    # =====================================================
    if active_vehicle.rc_expiry_date and active_vehicle.rc_expiry_date < month_end:
        errors.append("RC expired for claim month")

    if active_vehicle.insurance_expiry_date and active_vehicle.insurance_expiry_date < month_end:
    ...
    existing_claim = (
        ...
        .filter(
            ...
            or_(
                VehicleCMReimbursement.status == None,
                ~VehicleCMReimbursement.status.ilike('%rejected%') | ~VehicleCMReimbursement.status.ilike('%auto lapsed%')
            )
        )
        .first()
    )
    ...
    if existing_claim:
        errors.append(
            f"Claim already exists for {payload.claim_month_year}"
        )

    # =====================================================
    # ENTITLEMENT
    # =====================================================
    annual_fuel, annual_maintenance = ENTITLEMENTS[payload.employee_grade][vehicle_fuel_type]

    annual_fuel = Decimal(annual_fuel)
    annual_maintenance = Decimal(annual_maintenance)

    monthly_fuel = (annual_fuel / 12).quantize(Decimal("0.01"))
    monthly_maintenance = (annual_maintenance / 12).quantize(Decimal("0.01"))

    # =====================================================
    # FY USAGE (IMPORTANT 🔥)
    # =====================================================
    fuel_used = db.query(
        func.coalesce(func.sum(VehicleCMReimbursement.fuel_claimed_liters), 0)
        ).filter(
            VehicleCMReimbursement.created_by == payload.user_id,

            VehicleCMReimbursement.claim_month_year.between(
                fy_start.strftime("%Y-%m"),
                fy_end.strftime("%Y-%m")
            ),
    ...
    maintenance_used = db.query(
    func.coalesce(func.sum(VehicleCMReimbursement.maintenance_claim_amount), 0)
    ).filter(
        VehicleCMReimbursement.created_by == payload.user_id,
    ...
    # HARD CAPS
    # =====================================================
    if payload.fuel_claimed_liters > monthly_fuel:
        errors.append(f"Fuel exceeds monthly cap {monthly_fuel}")

    if payload.maintenance_claim_amount > monthly_maintenance:
        errors.append(f"Maintenance exceeds monthly cap {monthly_maintenance}")

    if fuel_used + payload.fuel_claimed_liters > annual_fuel:
        errors.append("Annual fuel entitlement exceeded")

    if maintenance_used + payload.maintenance_claim_amount > annual_maintenance:
        errors.append("Annual maintenance entitlement exceeded")
    ...
    result = {
        ...
        "annual_fuel_remaining": annual_fuel - (fuel_used + payload.fuel_claimed_liters),
        "annual_maintenance_remaining": annual_maintenance - (
            maintenance_used + payload.maintenance_claim_amount
        ),
        "monthly_fuel_cap": monthly_fuel,
        "monthly_maintenance_cap": monthly_maintenance,
    }
