from sqlalchemy import Column, Integer, String, Date, Time, DateTime, Boolean, Text, ForeignKey, relationship
from app.database import Base
from datetime import datetime


class WorkAtHeightPermit(Base):
    __tablename__ = "work_at_height_permit"

    whp_id = Column(Integer, primary_key=True, autoincrement=True)
    jsa_id = Column(Integer, ForeignKey("job_safety_analysis.jsa_id"), nullable=True)
    ...
    sc16_workers_trained_safety_belts = Column(String(20), nullable=True) 
    ...
    sc20_condition_fav_elevation_work = Column(String(20), nullable=True) 
    ...
    issuer_userid = Column(Integer, nullable=True)
    receiver_userid = Column(Integer, nullable=True)

    gas_requestor_userid = Column(Integer, nullable=True)
    gas_issuer_userid = Column(Integer, nullable=True)
    gas_receiver_userid = Column(Integer, nullable=True)

    closure_requestor_userid = Column(Integer, nullable=True)
    closure_issuer_userid = Column(Integer, nullable=True)
    closure_receiver_userid = Column(Integer, nullable=True)
    ...
    updated_at = Column(
      ...
      # NEW — child permits linked to this JSA
    work_at_height_permits = relationship(
        "WorkAtHeightPermit",
        back_populates="jsa",
