...
class MoCRequestHistory(Base):
    __tablename__ = "moc_request_history"
    ...
    # ✅ Correct Foreign Key — refers to moc_closure_id, not id
    moc_closure_id = Column(Integer, ForeignKey("moc_closures.moc_closure_id"), nullable=True)
    ...
    # ✅ Relationship to MoCRequest
    moc_request = relationship("MoCRequest", back_populates="histories")

    # ✅ Relationship to MoCClosure (using FK)
    closure = relationship("MoCClosure", back_populates="histories")

    # ✅ View-only relationship to HIRAEntry (safe)
    hira_entries = relationship(
        "HIRAEntry",
        primaryjoin="foreign(HIRAEntry.moc_request_id) == MoCRequestHistory.moc_request_id",
        viewonly=True
    )

    def __repr__(self):
        return f"<MoCRequestHistory(id={self.id}, moc_request_no='{self.moc_request_no}', status='{self.status}')>"
