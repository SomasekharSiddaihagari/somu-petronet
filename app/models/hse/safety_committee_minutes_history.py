...
class SafetyCommitteeMinutesHistory(Base):
    __tablename__ = "safety_committee_minutes_history"
 
    history_id = Column(Integer, primary_key=True, index=True)
    scm_id = Column(Integer, nullable=True)  # 🔥 store original minutes ID for reference
    station_id = Column(Integer, nullable=True)
    meeting_no = Column(String(100), nullable=False)
    location = Column(String(255))
    frequency = Column(String(100))
    meeting_date = Column(Date)
    next_meeting = Column(Date)
