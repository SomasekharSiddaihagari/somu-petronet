...
def _cols_for(lt: dict) -> int:
    """EL types get 4 cols (available, availed, encashed, auto_encashed), rest get 2."""
    return 4 if lt["code"].lower() in {"el_e", "el_ne"} else 2
...
def generate_leave_report_excel(report: LeaveReportResponse) -> bytes:
    ...
    FIXED_COLS = 3
    leave_types = report.leave_type_meta

    # Calculate start column for each leave type dynamically
    lt_start_cols = {}
    cur_col = FIXED_COLS + 1
    for lt in leave_types:
        lt_start_cols[lt["code"]] = cur_col
        cur_col += _cols_for(lt)
    ...
    for i in range(FIXED_COLS + 1, total_cols + 1):
    ...
    for col_idx, label in enumerate(["EMP ID", "EMP Name", "Station"], start=1):
        ...
        ws.merge_cells(start_row=4, start_column=col_idx,
                       end_row=5,   end_column=col_idx)

    # ── Row 4 – Leave type group headers ──────────────────────────────────────
    ENCASHED_BG = "FFF2CC"
    ENCASHED_FG = "7F6000"

    for lt in leave_types:
        start_col = lt_start_cols[lt["code"]]
        end_col   = start_col + _cols_for(lt) - 1
        ws.merge_cells(
    ...
    for lt in leave_types:
        start_col  = lt_start_cols[lt["code"]]
        is_el      = lt["code"].lower() in {"el_e", "el_ne"}
        ...
        if is_el:
            sub_cols.append(
                (start_col + 2, "Encashed\nLeave",      ENCASHED_BG, ENCASHED_FG)
            )
            sub_cols.append(
                (start_col + 3, "Auto\nEncashed",        AUTO_ENC_BG, AUTO_ENC_FG)
            )

        for col, label, bg, fg in sub_cols:
    ...
    note.value = (
        "Available = balance as on End Date  |  "
        "Availed = days taken in period  |  "
        "Encashed = EL days encashed (approved)  |  "
        "Auto Encashed = EL days encashed automatically"
    )
    ...
    for idx, rec in enumerate(report.records):
        ...
        for lt_meta in leave_types:
            code      = lt_meta["code"]
            start_col = lt_start_cols[code]
            summary   = lt_map.get(code)
            is_el     = code.lower() in {"el_e", "el_ne"}

            write(start_col,     float(summary.available) if summary else 0.0,
                  bg=AVAIL_BG,    num_fmt="0.00")
            write(start_col + 1, float(summary.availed)   if summary else 0.0,
                  bg=AVAILED_BG,  num_fmt="0.00")
            if is_el:
                write(start_col + 2, float(summary.encashed)      if summary else 0.0,
                      bg=ENCASHED_BG, num_fmt="0.00")
                write(start_col + 3, float(summary.auto_encashed)  if summary else 0.0,
                      bg=AUTO_ENC_BG, num_fmt="0.00")
    ...
    if report.records:
        ...
        for lt_meta in leave_types:
            ...
            start_col = lt_start_cols[code]
            is_el     = code.lower() in {"el_e", "el_ne"}
            num_subcols = 4 if is_el else 2

            for offset in range(num_subcols):
