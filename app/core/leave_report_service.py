...
from app.schemas.leave.leave_report import (
    LeaveDateDetail, LeaveDatesResponse, LeaveReportResponse, LeaveTypeSummary, UserLeaveDatesReport, UserLeaveReport
)
...
def get_leave_report(
    ...
    consumed_rows = db.execute(text(f"""
        SELECT
            ...
            SUM(
                CASE 
                    WHEN LOWER(la.status) = 'withdraw approved' THEN 0
                    ELSE (CASE WHEN lad.day_type = 'half' THEN 0.5 ELSE 1 END) 
        ...
        AND la.to_date + INTERVAL '7 days' <= CURRENT_DATE   -- ✅ same 7-day window as summary
        {uid_la}
        GROUP BY la.user_id, UPPER(lt.code)
    """), params).mappings().all()

    # ── 3a. EL encashment (Approved only) ────────────────────────────────────────
    ...
    # ── 3b. EL auto encashment ────────────────────────────────────────────────────
    auto_encashed_rows = db.execute(text(f"""
        SELECT
            COALESCE(em.created_by, le.created_by) AS user_id,
            SUM(le.encash_el)                       AS total_auto_encashed
        FROM leave_encashment le
        LEFT JOIN encashment_main em
            ON em.encashment_main_id = le.encashment_main_id
        WHERE LOWER(le.status) = 'auto encashed'
        AND le.encashment_date <= :to_date
        {uid_enc}
        GROUP BY COALESCE(em.created_by, le.created_by)
    """), params).mappings().all()
    ...
    auto_encashed_map: dict[int, Decimal] = {}
    ...
    for row in auto_encashed_rows:
        uid = row["user_id"]
        auto_encashed_map[uid] = auto_encashed_map.get(uid, Decimal(0)) + Decimal(
            str(row["total_auto_encashed"] or 0)
        )
    ...
    for uid in sorted(all_uids):
        ...
        auto_enc_el  = auto_encashed_map.get(uid, Decimal(0))
        total_enc_el = enc_el + auto_enc_el  
        ...
        el_e_available  = max(el_e_alloc  - used_e  - total_enc_el, Decimal(0))
        ...
        el_auto_encashed_override = {
            "EL_E":  auto_enc_el,  # auto encashed shown in new column
            "EL_NE": Decimal(0),
        }
        # ─────────────────────────────────────────────────────────────────────
        ...
        for lt in all_leave_types:
            ...
            if is_el_type:
                ...
                auto_encashed_days = el_auto_encashed_override[code].quantize(Decimal("0.01"))
            else:
            ...
            leave_summaries.append(LeaveTypeSummary(
                ...
                auto_encashed=auto_encashed_days,   # ← new field
            ))
...
def get_leave_dates_report(
    ...
    # Dynamic filters
    uid_filter = "AND la.user_id = :user_id" if user_id else ""
    station_filter = "AND LOWER(s.station_name) = LOWER(:station)" if station else ""
    leave_type_filter = ""
    
    if leave_type:
        leave_type_filter = """
            AND (LOWER(lt.code) = LOWER(:leave_type) 
                 OR LOWER(lt.name) = LOWER(:leave_type))
        """
    ...
    query = text(f"""
        SELECT 
            ...
            TRIM(CONCAT(u.first_name, ' ', u.last_name)) AS emp_name,
            s.station_name,
            s.station_id,
            lad.leave_date,
            lad.day_type,
            UPPER(lt.code) AS leave_type_code,
            lt.name AS leave_type_name,
            la.status,
            la.leave_id as leave_application_id
        ...
        JOIN leave_types lt ON (
            LOWER(lt.name) = LOWER(la.leave_type) 
            OR LOWER(lt.code) = LOWER(la.leave_type)
        )
        JOIN users u ON u.user_id = la.user_id
        LEFT JOIN station s ON s.station_id = u.station_id
        WHERE lad.leave_date BETWEEN :from_date AND :to_date
            AND lt.is_active = TRUE
            AND u.is_deleted = FALSE
            AND LOWER(la.status) IN ({_in(CONSUMING_STATUSES)})
            AND la.to_date + INTERVAL '7 days' <= CURRENT_DATE
            {uid_filter}
            {station_filter}
            {leave_type_filter}
        ORDER BY la.user_id, lad.leave_date
    """)
    ...
    for row in rows:
        ...
        user_data_map[uid]["leave_dates"].append(leave_detail)
        
        # Add to total (0.5 for half day, 1 for full day)
        if row["day_type"] == "half":
            user_data_map[uid]["total_days"] += Decimal("0.5")
        else:
            user_data_map[uid]["total_days"] += Decimal("1")
    
    # Build records
    records = []
    for uid in sorted(user_data_map.keys()):
        user_data = user_data_map[uid]
        records.append(UserLeaveDatesReport(
            emp_id=user_data["emp_id"],
            emp_code=user_data["emp_code"],
            emp_name=user_data["emp_name"],
            station=user_data["station"],
            station_id=user_data["station_id"],
            leave_dates=user_data["leave_dates"],
            total_days=user_data["total_days"]
        ))
    
    return LeaveDatesResponse(
        from_date=from_date,
        to_date=to_date,
        station=station,
        total_employees=len(records),
